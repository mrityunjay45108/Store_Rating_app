...
export default function OwnerDashboard() {
  ...
  return (
    <Container size="xl" py="xl">
      ...
      {data?.stats && <OwnerStats stats={data.stats} />}

      <Paper withBorder p="md" radius="md" shadow="sm" mt="xl">
        <Group mb="md" justify="space-between">
          <Group>
            <IconUsers size={20} />
            <Title order={4}>Recent Reviewers</Title>
          </Group>
            ...
            <Table.Tbody>
              {data?.reviews?.length > 0 ? (
                data.reviews.map((rev, index) => (
                  <Table.Tr key={index}>
                    <Table.Td fw={500}>{rev.user_name}</Table.Td>

                    <Table.Td>{rev.email}</Table.Td>

                    <Table.Td>
                      <Badge
                        color={rev.rating >= 4 ? "green" : "yellow"}
                        variant="light"
                      >
                        {rev.rating} ★
                      </Badge>
                    ...
                    <Table.Td>{rev.comment || "No comment"}</Table.Td>
                ...
                <Table.Tr>
                  <Table.Td colSpan={5} align="center" py="xl">
                    <Text c="dimmed">No ratings submitted yet.</Text>
                  </Table.Td>
                </Table.Tr>
              )}
            </Table.Tbody>
